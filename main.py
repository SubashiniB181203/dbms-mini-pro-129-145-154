...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/dbms9'
db=SQLAlchemy(app)
...
@app.route('/addcgpa',methods=['POST','GET'])
def addcgpa():
    # query=db.engine.execute(f"SELECT * FROM `student`") 
    query=Student.query.all()
    if request.method=="POST":
        ...
        print(cgp,rollno)
        cg=Cgpa(rollno=rollno,cgpa=cgp)
        db.session.add(cg)
        db.session.commit()
        flash("CGPA added","warning")

        
    return render_template('cgpa.html',query=query)
...
def search():
    if request.method=="POST":
        ...
        cgp=Cgpa.query.filter_by(rollno=rollno).first()
        return render_template('search.html',bio=bio,attend=attend,cgp=cgp)
...
def addstudent():
    ...
    if request.method=="POST":
        ...
        query=Student(rollno=rollno,sname=sname,dob=dob,sem=sem,gender=gender,branch=branch,email=email,number=num,fname=fname,mname=mname,address=address)
